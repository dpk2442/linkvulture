...
class Site(object):
    ...
    def __init__(self, url, remote=False, depth=0):
        ...
        for link in soup.find_all("a"):
            ...
            is_valid = IS_VALID_URL.match(theURL)
            if theURL not in VISITED and is_valid is not None:
                toProcess.append(theURL)
                VISITED.append(theURL)
        for theURL in toProcess:
            self.children.append(
                Site(theURL, IS_LOCAL.match(theURL) is None, depth + 1))
    ...
    def __str__(self):
        ret = ""
        if self.code != 200 or not args.errors or \
           (args.parents and self.has404Children()):
            ret += str(self.code) + " => " + self.url
        if self.children:
            for child in self.children:
                carr = str(child).split("\n")
                carr = [i for i in carr if EMPTY_OUTPUT_LINE.match(i) is None]
                carr.insert(0, "")
                ret += ("\n" + args.tree).join(carr)
        return ret
...
if __name__ == "__main__":
    ...
    tree_group.add_argument(
        "-n", "--no-tree", action="store_true",
        help="Use this option to disable the printing of the tree")
    tree_group.add_argument(
        "-t", "--tree", metavar="chars", default="| -- ",
        help="Set the characters for the tree. Defaults to \"| -- \"")
    parser.add_argument(
        "-e", "--errors", action="store_true",
        help="Show only errors")
    parser.add_argument(
        "-p", "--parents", action="store_true",
        help="Show the parents of pages with errors")
    parser.add_argument(
        "-m", "--max-depth", type=int, metavar="depth", default=float("inf"),
        help="Set the max recursion depth. Defaults to infinite.")
    parser.add_argument(
        "url",
        help="The url to map")
